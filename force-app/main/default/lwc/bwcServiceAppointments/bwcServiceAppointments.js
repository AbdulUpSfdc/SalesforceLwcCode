...
const COMPONENT_UI_NAME = 'Service Appointments';

export default class BwcServiceAppointments extends BwcPageElementBase {
    ...
    async getAppointments() {
        try {
            ...
            if (this.authorizedBans.length > 0) {
                ...
                let sortedData = await BwcAppointmentsService.getByDefaultSortedAppointments(this.recordId, this.authorizedBans, BwcAppointmentsService.APPOINTMENT_TYPE_SERVICE, this.defaultSortBy, this.defaultSortDirection);
                this.serviceAppointmentsData = sortedData.slice(0, 5);
                if (this.serviceAppointmentsData.length > 0) {
    ...
    sortAppointmentsData(fieldname, direction) {
        let parseData = JSON.parse(JSON.stringify(this.serviceAppointmentsData));
        let sortedData = BwcAppointmentsService.getSortedAppointments(parseData, fieldname, direction);
        this.serviceAppointmentsData = sortedData;
    }

    //Navigating to bwcServiceAppointmentList component on click of View All
    handleNavigate(event) {
        const pageReference = {
            type: 'standard__component',
            attributes: {
                componentName: 'c__BWCServiceAppointmentListWrapper'
            },
            state: {
                c__recordId: this.recordId
            }
        };
        super.openSubtab(pageReference, 'Service Appointments', 'custom:custom40');
    }

    //Handles WFE tech Support button Click
    handleWFETechSupportButtonClick() {
        this.template.querySelector('c-bwc-launch-w-f-e').open(InteractionActivityValueMapping.ManageServiceAppointments, undefined, undefined, undefined, this.recordId);
    }
}